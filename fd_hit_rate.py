...
File name: fd_hit_rate.py

Description: Compute hit rates for front-door adjustment using exhaustive search.
Tests proposed method from Theorem 3.2 on random graphs for Table 1.
Searches all possible subsets z to find valid front-door-like adjustments.
"""
...
from supporting_func import powerset, check_additional_CI

def main(args):
    """Test exhaustive search method for finding valid front-door adjustments on random graphs."""
    ...
    ours = np.zeros((len(number_observed_list),number_iterations))
    ...
    for n_count, number_observed in enumerate(number_observed_list):
        ...
        for iter in range(number_iterations):
            ...
            for candidate_z in powerset(possible_z):
    ...
    print('------------------------------------------------------------')
    print('Run time for our method: ' + str(time.perf_counter() - starting_time))
    print(np.sum(ours, axis = 1)/number_iterations)
    print('------------------------------------------------------------')
    
    with open(output_path + '_ours.txt', 'a') as sys.stdout:
        print("Our method with d = " + str(degree) + " for Table " + str(table))
        print(np.sum(ours, axis = 1)/number_iterations)
        
    np.save(output_path + '_ours.npy', ours)
    np.save(output_path + '_time_ours.npy', np.array(time.perf_counter() - starting_time))
